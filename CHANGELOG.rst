...
.. warning::
    ...
    and it is required to first update to Objects API version 3.6.0 and run an import command
...
.. warning::
  ...
  using ``maykinmedia/objects-api``, make sure to change to the new repository.

  TODO mention other renames: github repo, RTD, OTEL_SERVICE_NAME?
...
* [:commonground-api-common:`142`] Use exception handler registry from commonground-api-common <change in error format>
* Change namespace for OpenTelemetry metrics from ``objects`` to ``openobject`` and add metrics for Objecttypes (see :ref:`installation_observability_metrics`)
* Changes to ``setup_configuration`` (see :ref:`installation_config_cli`)

  * Remove the ``SitesConfigurationStep`` (namespace ``sites_config``)
